...
func (self *Instruction) String() string {
	if self == nil {
		...
		return fmt.Sprintf("<%p>{name=%s, argc=%d, argv=%v, domain=%d, pseudo.kind=%d}",
			self, self.name, self.argc, self.argv[:self.argc], self.domain, self.pseudo.kind)
	}
}
...
var linkHistory sync.Map

// Link pins a label at the current position.
func (self *Program) Link(p *Label) {
	if p.Dest != nil {
		info := fmt.Sprintf("lable was alreay linked: Label=%s, Label.Dest=%s", p, p.Dest)
		if h, ok := linkHistory.Load(p); ok {
			info += fmt.Sprintf(", prev_stack=[[[%s]]]", h.(*history).Stack)
		}
		panic(info)
	} else {
		p.Dest = self.pseudo(_PseudoNop)
		linkHistory.Store(p, &history{Stack: debug.Stack()})
	}
}
